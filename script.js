...
const toutesCategories = ["Féculent", "Produit laitier", "Viande", "Poisson", "Fruit", "Légume", "Sucré", "Sauce", "Epice", "Boisson", "Conserve", 
                          "Produit du monde", "Hygiène"];
...
function genererListeCourses() {
  ...
  const totalDemandes = {};
  const achatsInputs = {}; // ← stocke les champs "acheté"
  ...
  btnAjouterInventaire.textContent = "➕ Ajouter à l'inventaire";
  ...
  if (Object.keys(courses).length === 0) {
    container.innerHTML += "<p>✅ Tout est déjà dans l'inventaire.</p>";
    return;
  }
  const ordreCategories = [
    "Autres",
    "Féculent", "Produit laitier", "Viande", "Poisson",
    "Fruit", "Légume", "Sucré", "Sauce",
    "Epice", "Boisson", "Conserve", "Produit du monde", "Hygiène"
  ];
...
function nouvelleSemaine() {
  if (!confirm("Voulez-vous vraiment commencer une nouvelle semaine ? Toutes les cases seront vidées.")) return;
  ...
  alert("Nouvelle semaine prête ! Toutes les cases ont été vidées.");
}
function repasCuisiné() {
  if (!confirm("Retirer les ingrédients du stock et marquer le repas comme fait ?")) return;
  ...
  genererListeCourses();
  fermerPopupRepas();
  alert("✅ Repas validé, stock mis à jour !");
}
...
const tachesProprete = {
  ...
  "Salon": ["Rangement", "Aspirateur", "Serpillère"],
...
function resetProprete() {
  if (!confirm("Commencer une nouvelle semaine ? Toutes les cases seront décochées.")) return;
...
function genererGraphique(){
  ...
  if(total === 0){
    ...
    legend.innerHTML = "<li>Aucune dépense enregistrée</li>";
    return;
  }
  ...
  for(const cat in data){
    ...
    ctx.fill();
    const li = document.createElement("li");
    li.textContent = `${cat} : ${data[cat].toFixed(2)} €`;
...
function afficherDepenses() {
  ...
  Object.keys(depensesTriees).forEach(categorie => {
    ...
    depensesTriees[categorie].forEach(d => {
      const li = document.createElement("li");
      const span = document.createElement("span");
      span.textContent = `${d.note} : ${d.montant} €`;
      const btnSupprimer = document.createElement("button");
      btnSupprimer.textContent = "Supprimer";
      btnSupprimer.style.padding = "10px 16px";
      btnSupprimer.style.fontSize = "1em";
...
function afficherHistorique() {
  ...
  mois.forEach(m => {
    ...
    const span = document.createElement("span");
    span.textContent = `${m} — ${total.toFixed(2)} €`;
...
function ouvrirPopupHistorique(mois) {
  ...
  document.getElementById("historiqueTitre").textContent = "Détails " + mois;
